...
function updateCropPreview() {
  ...
  if (eyebrow)
    eyebrow.textContent = currentLanguage === "en" ? "Selected Crop" : "Tanaman Terpilih";
...
function localizeNavigationLabels(language = currentLanguage) {
  ...
  setButtonTextAfterIcon(document.getElementById("nav-mobile-beranda"), labels.mobileBeranda);
  setButtonTextAfterIcon(document.getElementById("nav-mobile-perhitungan"), labels.mobilePerhitungan);
...
function localizedMessage(message) {
  ...
  let match = message.match(/^Error: Waktu harus lebih besar dari (.+) menit!$/);
...
function showToast(message, isError = false) {
  ...
  toastMsg.innerText = displayMessage;
...
document.getElementById("increaseWaktu")?.addEventListener("click", () => adjustSensorValue("inputWaktu", 1));
document.getElementById("decreaseDebit")?.addEventListener("click", () => adjustSensorValue("inputDebit", -1));
document.getElementById("increaseDebit")?.addEventListener("click", () => adjustSensorValue("inputDebit", 1));
...
document.getElementById("decreaseKebutuhan")?.addEventListener("click", () => adjustRequirementValue(-1));
document.getElementById("increaseKebutuhan")?.addEventListener("click", () => adjustRequirementValue(1));
...
function updateUI() {
  ...
  const target = lahan * air;

  const elVolume = document.getElementById("cardVolume");
  if (elVolume) {
      elVolume.innerText = current.toLocaleString(currentLanguage === "en" ? "en-US" : "id-ID");
  }

  const elTarget = document.getElementById("cardTarget");
  if (elTarget) {
      elTarget.innerText = target.toLocaleString(currentLanguage === "en" ? "en-US" : "id-ID");
  }
  ...
  if (elPercent) {
      elPercent.innerText = `${displayPercent.toFixed(1)}%`;
  }

  const elProgress = document.getElementById("progressBar");
  if (elProgress) {
      elProgress.style.width = `${displayPercent}%`;
  }
  ...
  const submitBtn = document.querySelector('#formDebit button[type="submit"]') || document.querySelector('#formDebit button');
  ...
  if (inputWaktuElem && inputDebitElem && submitBtn) {
    if (isFull) {
      ...
      sensorStepperButtons.forEach((button) => { button.disabled = true; });
      ...
      sensorStepperButtons.forEach((button) => { button.disabled = false; });
...
function getSupportValidationMessage(report, checkAntiSpam = true) {
  ...
  if (checkAntiSpam && Date.now() - SUPPORT_FORM_READY_AT < SUPPORT_MIN_READY_MS) {
...
function buildSupportFormData(report) {
  ...
  formData.append("waktu_laporan", new Date(report.createdAt).toLocaleString("id-ID"));
  formData.append("_subject", `[Smart Irrigation Support] ${report.type} - ${report.page}`);
  return formData;
}
...
async function sendSupportReportToFormspree(report) {
  ...
  if (!response.ok) {
    throw new Error("Laporan gagal dikirim. Cek endpoint Formspree atau koneksi internet.");
  }
}
...
function getSupportFieldMessage(field) {
  ...
  if (validity.valueMissing)
    return currentLanguage === "en" ? `${label} is required.` : `${label} wajib diisi.`;
...
function exportSupportReports() {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = currentLanguage === "en" ? "Smart_Irrigation_Support_Reports.json" : "Laporan_Support_Smart_Irrigation.json";
  a.click();
